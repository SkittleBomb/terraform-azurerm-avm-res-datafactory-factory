variable "name" {
  ...
  validation {
    condition = (
      can(regex("^[a-zA-Z0-9]([-a-zA-Z0-9]{1,61}[a-zA-Z0-9])?$", var.name)) &&
      length(var.name) >= 3 && length(var.name) <= 63
    )
    error_message = "The name must start with a letter or a number, can contain only letters, numbers, and the dash (-) character. Every dash (-) character must be immediately preceded and followed by a letter or a number. Consecutive dashes are not permitted in container names. Name can be 3-63 characters long."
  }
}
...
variable "global_parameter" {
  ...
  validation {
    condition = alltrue([
      for gp in var.global_parameter : contains(["Array", "Bool", "Float", "Int", "Object", "String"], gp.type)
    ])
    error_message = "Each global parameter type must be one of the following: Array, Bool, Float, Int, Object, String."
  }
}
...
variable "identity" {
  ...
  validation {
    ...
    error_message = "The type must be one of the following: SystemAssigned, UserAssigned, SystemAssigned, UserAssigned."
  }
}
